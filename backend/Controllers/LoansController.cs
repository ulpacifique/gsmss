...
namespace CommunityFinanceAPI.Controllers
{
    ...
    public class LoansController : ControllerBase
    {
        ...
        private readonly ILoanRiskAssessmentService _riskAssessmentService;
        private readonly ILogger<LoansController> _logger;

        public LoansController(ILoanService loanService, ILoanPaymentService loanPaymentService, ILoanRiskAssessmentService riskAssessmentService, ILogger<LoansController> logger)
        {
        ...
        public async Task<IActionResult> RequestLoan([FromBody] CreateLoanRequest request)
        {
            try
            {
                ...
                // Perform AI risk assessment before creating loan
                var riskAssessment = await _riskAssessmentService.AssessLoanRiskAsync(user.UserId, request.Amount, request.Purpose ?? "");
                
                var loan = await _loanService.RequestLoanAsync(user.UserId, request);
                _logger.LogInformation("User {UserId} requested loan {LoanId} with risk level {RiskLevel} (Score: {RiskScore})", 
                    user.UserId, loan.LoanId, riskAssessment.RiskLevel, riskAssessment.RiskScore);
                
                // Return loan with risk assessment
                return Ok(new { 
                    Loan = loan, 
        ...
        public async Task<IActionResult> GetLoanPayments(int loanId)
        {
            try
            {
                ...
                var payments = await _loanPaymentService.GetLoanPaymentsAsync(loanId);
                return Ok(payments);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error getting loan payments");
                return StatusCode(500, new { message = "An error occurred while retrieving loan payments" });
            }
        }

        [HttpGet("payments/my-payments")]
        public async Task<IActionResult> GetMyLoanPayments()
        {
            try
            {
                if (HttpContext.Items["AuthenticatedUser"] is not Models.Entities.User user)
                ...
                var payments = await _loanPaymentService.GetUserLoanPaymentsAsync(user.UserId);
                return Ok(payments);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error getting user loan payments");
                return StatusCode(500, new { message = "An error occurred while retrieving your loan payments" });
            }
        }

        [HttpGet("payments/{paymentId}")]
        public async Task<IActionResult> GetLoanPaymentById(int paymentId)
        {
            try
            {
                if (HttpContext.Items["AuthenticatedUser"] is not Models.Entities.User user)
                ...
                var payment = await _loanPaymentService.GetLoanPaymentByIdAsync(paymentId);
            ...
            {
                _logger.LogError(ex, "Error getting loan payment");
                return StatusCode(500, new { message = "An error occurred while retrieving loan payment" });
            }
        }

        // AI Risk Assessment endpoints
        [HttpPost("assess-risk")]
        public async Task<IActionResult> AssessLoanRisk([FromBody] AssessRiskRequest request)
        {
            try
            {
                if (HttpContext.Items["AuthenticatedUser"] is not Models.Entities.User user)
                ...
                var assessment = await _riskAssessmentService.AssessLoanRiskAsync(user.UserId, request.Amount, request.Purpose ?? "");
                return Ok(assessment);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error assessing loan risk");
                return StatusCode(500, new { message = "An error occurred while assessing loan risk" });
            }
        }

        [HttpGet("risk-score")]
        public async Task<IActionResult> GetUserRiskScore()
        {
            try
            {
                if (HttpContext.Items["AuthenticatedUser"] is not Models.Entities.User user)
                ...
                var riskScore = await _riskAssessmentService.GetUserRiskScoreAsync(user.UserId);
                return Ok(riskScore);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error getting user risk score");
                return StatusCode(500, new { message = "An error occurred while retrieving risk score" });
            }
        }
    }
