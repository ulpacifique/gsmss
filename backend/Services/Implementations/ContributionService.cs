...
namespace CommunityFinanceAPI.Services.Implementations
{
    public class ContributionService : IContributionService
    {
        ...
        public async Task<ContributionResponse> CreateContributionAsync(CreateContributionRequest request, int userId)
        {
            ...
            // Check contribution limits for this goal
            var limit = await _context.ContributionLimits
                .FirstOrDefaultAsync(cl => cl.GoalId == request.GoalId && cl.IsActive);

            if (limit != null)
            {
                // Check fixed amount (treated as minimum required amount for community finance flexibility)
                // This allows members to contribute more than the fixed amount, which is common in community finance
                if (limit.FixedAmount.HasValue && request.Amount < limit.FixedAmount.Value)
                    throw new InvalidOperationException($"Contribution amount must be at least {limit.FixedAmount.Value:C} for this goal. The fixed amount is {limit.FixedAmount.Value:C}, but you can contribute more if you wish.");

                // Check minimum amount (only if fixed amount is not set)
                if (!limit.FixedAmount.HasValue && limit.MinimumAmount.HasValue && request.Amount < limit.MinimumAmount.Value)
                    throw new InvalidOperationException($"Contribution amount must be at least {limit.MinimumAmount.Value:C} for this goal.");

                // Check maximum amount per contribution
                if (limit.MaximumAmount.HasValue && request.Amount > limit.MaximumAmount.Value)
                    throw new InvalidOperationException($"Contribution amount cannot exceed {limit.MaximumAmount.Value:C} for this goal.");

                // Check maximum total per user for this specific goal
                if (limit.MaximumTotalPerUser.HasValue)
                {
                    var userTotalContributionsToThisGoal = await _context.Contributions
                        .Where(c => c.UserId == userId && c.GoalId == request.GoalId && c.Status == "Approved")
                        .SumAsync(c => (decimal?)c.Amount) ?? 0m;

                    if (userTotalContributionsToThisGoal + request.Amount > limit.MaximumTotalPerUser.Value)
                        throw new InvalidOperationException($"Your total contributions to this goal cannot exceed {limit.MaximumTotalPerUser.Value:C}. Current total for this goal: {userTotalContributionsToThisGoal:C}");
                }
            }
